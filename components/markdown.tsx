...
export default function Markdown({ markdown, className }: MarkdownProps) {
    const convertToHtml = (markdownText: string) => {
        ...
        markdownText = markdownText.replace(
            /^## (.*$)/gim,
            '<h2 class="text-2xl font-semibold">$1</h2>'
        )
        markdownText = markdownText.replace(
            /^# (.*$)/gim,
            '<h1 class="text-3xl font-bold mb-0">$1</h1>'
        )
        ...
        markdownText = markdownText.replace(
            /^(?:-|\+|\*)\s+(.+)$/gim,
            '<li class="list-disc ml-5 text-[13px]">$1</li>'
        )
        markdownText = markdownText.replace(
        ...
        markdownText = markdownText.replace(
            /^\d+\.\s+(.+)$/gim,
            '<li class="list-decimal ml-5 text-[13px]">$1</li>'
        )
        markdownText = markdownText.replace(
